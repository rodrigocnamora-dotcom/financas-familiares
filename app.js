...
const BANK_HINTS = {
  ...
  santander: 'Santander: Movimentos → Exportar → XLS',
  millennium:'Millennium: Consultas → Movimentos → Exportar XLS ou CSV'
};
const BANK_COLS = {
  revolut:   { date: 'Data de início', desc: 'Descrição', amount: 'Montante' },
  santander: { date: 'Data Operação',  desc: 'Descrição', amount: 'Montante( EUR )' },
  millennium:{ date: 'Data',           desc: 'Descrição', amount: 'Valor' }
};

function cssColor(cat) {
  const v = CAT_COLOR_VAR[cat];
  if (v) return getComputedStyle(document.documentElement).getPropertyValue(v).trim();
  return '#888780';
}

// categorias em memória (podem ser editadas pelo utilizador)
let customCats = [...DEFAULT_CATS];

function getCats()      { return customCats; }
function getCatNames()  { return customCats.map(c => c.name); }
...
let transactions = [], rules = {}, budgets = {}, ownAccounts = [];
let currentMonth = new Date().getMonth();
let currentYear  = new Date().getFullYear();
let activeScreen = 'resumo', selectedBank = 'revolut';
let connectionOk = false, channelsSubscribed = false;
...
async function init() {
  ...
  session = s;
  supabaseClient.auth.onAuthStateChange(async (_e, s2) => {
    session = s2;
    if (session) {
  ...
  if (session) { await checkAdminStatus(); await loadAllData(); subscribeRealtime(); }
  renderApp();
}
...
async function loadAllData() {
  try {
    const [txRes, rulesRes, budgetsRes, accRes] = await Promise.all([
    ...
    if (txRes.error || rulesRes.error || budgetsRes.error || accRes.error) throw new Error();
    transactions = txRes.data.map(rowToTx);
    rules = {}; rulesRes.data.forEach(r => rules[r.keyword] = r.category);
    budgets = {}; budgetsRes.data.forEach(b => budgets[b.category] = b.monthly_amount);
    ownAccounts = accRes.data;
    connectionOk = true;
  } catch {
    connectionOk = false;
    showToast('Não foi possível ligar à base de dados.');
  }
}

function rowToTx(row) {
  return { id: row.id, date: row.date, desc: row.description, amount: row.amount,
...
function subscribeRealtime() {
  ...
  const tables = ['transactions','rules','budgets','own_accounts'];
  tables.forEach(t => {
    supabaseClient.channel('public:'+t)
      .on('postgres_changes', { event: '*', schema: 'public', table: t }, p => {
        handleRealtimeChange(t, p);
      }).subscribe();
  });
}
...
function handleRealtimeChange(table, payload) {
  if (table === 'transactions') {
    if (payload.eventType === 'INSERT') {
      if (!transactions.some(t => t.id === payload.new.id)) transactions.unshift(rowToTx(payload.new));
    } else if (payload.eventType === 'UPDATE') {
      const i = transactions.findIndex(t => t.id === payload.new.id);
      if (i !== -1) transactions[i] = rowToTx(payload.new);
    } else if (payload.eventType === 'DELETE') {
      transactions = transactions.filter(t => t.id !== payload.old.id);
    }
  } else if (table === 'rules') {
    if (payload.eventType === 'DELETE') delete rules[payload.old.keyword];
    else rules[payload.new.keyword] = payload.new.category;
  } else if (table === 'budgets') {
    if (payload.eventType !== 'DELETE') budgets[payload.new.category] = payload.new.monthly_amount;
  } else if (table === 'own_accounts') {
    if (payload.eventType === 'DELETE') ownAccounts = ownAccounts.filter(a => a.id !== payload.old.id);
    else if (payload.eventType === 'INSERT') { if (!ownAccounts.some(a => a.id === payload.new.id)) ownAccounts.push(payload.new); }
    else if (payload.eventType === 'UPDATE') { const i = ownAccounts.findIndex(a => a.id === payload.new.id); if (i !== -1) ownAccounts[i] = payload.new; }
  }
  if (['resumo','graficos','transacoes','transferencias','regras','ficheiros'].includes(activeScreen)) renderScreen(activeScreen);
}
...
function matchRule(desc) {
  ...
  for (const [kw, cat] of Object.entries(rules)) { if (d.includes(kw)) return { cat, kw }; }
  return null;
}
function detectInternalTransfer(desc) {
  const cleaned = desc.replace(/\s+/g,'').toUpperCase();
  return ownAccounts.some(a => cleaned.includes(a.iban_fragment.replace(/\s+/g,'').toUpperCase()));
}
...
function parseCSV(text, bank) {
  ...
  if (dateIdx === -1 || descIdx === -1 || amountIdx === -1)
    return { error: `Não encontrei as colunas esperadas para ${bank} ("${cols.date}", "${cols.desc}", "${cols.amount}"). Confirma se selecionaste o banco certo.` };
  const rows = [];
...
function parseXLS(rows, bank) {
  ...
  for (let i = 0; i < Math.min(rows.length, 15); i++) {
    ...
    if (row.some(c => c.includes('Data')) && row.some(c => c.includes('Descri'))) { headerIdx = i; break; }
  }
  if (headerIdx === -1) return { error: `Não encontrei o cabeçalho no ficheiro ${bank}.` };
  ...
  for (let i = headerIdx + 1; i < rows.length; i++) {
    ...
    const amount = parseFloat(rawAmount);
    const date = normalizeDate(String(row[dateIdx]||'').trim());
    if (!desc || isNaN(amount) || amount === 0 || !date || date === 'undefined') continue;
    out.push({ date, desc, amount, bank: bank.charAt(0).toUpperCase()+bank.slice(1) });
  }
  if (!out.length) return { error: 'Nenhuma transação válida encontrada no ficheiro.' };
  return out;
}
...
function normalizeDate(raw) {
  ...
  // formato "29-06-2026" já tratado acima; tentar DD-MM-YYYY com traço
...
async function importTransactions(rows) {
  ...
  if (!rows.length) { showToast('Nenhuma transação válida encontrada.'); return; }
  const existing = new Set(transactions.map(txFingerprint));
  const fresh = []; let dupeCount = 0;
  rows.forEach(r => {
    const fp = txFingerprint(r);
    if (existing.has(fp)) { dupeCount++; return; }
    existing.add(fp);
    const isInternal = detectInternalTransfer(r.desc);
    const m = isInternal ? null : matchRule(r.desc);
    fresh.push({ date: r.date, description: r.desc, amount: r.amount, bank: r.bank,
                 category: isInternal ? null : (m ? m.cat : null),
                 auto_categorized: !!m, fingerprint: fp, is_internal_transfer: isInternal });
  });
  if (!fresh.length) { showToast(`Nenhuma transação nova (${dupeCount} já existiam).`); return; }
  const { data, error } = await supabaseClient.from('transactions').insert(fresh).select();
  if (error) { console.error(error); showToast('Erro ao guardar transações na base de dados.'); return; }
  ...
  showToast(`${fresh.length} transações importadas` + (internal ? `, ${internal} transferências` : '') + (dupeCount ? `, ${dupeCount} duplicados ignorados` : ''));
}

// ── Cálculos ──────────────────────────────────────────────────
function getMonthExpenses(m, y) {
  return transactions.filter(e => { const d = new Date(e.date); return d.getMonth()===m && d.getFullYear()===y && !e.isInternal && e.amount < 0; });
}
function getMonthIncome(m, y) {
  return transactions.filter(e => { const d = new Date(e.date); return d.getMonth()===m && d.getFullYear()===y && !e.isInternal && e.amount > 0; });
}
function getMonthInternalTransfers(m, y) {
  return transactions.filter(e => { const d = new Date(e.date); return d.getMonth()===m && d.getFullYear()===y && e.isInternal; });
}
function catTotal(exps, cat) { return exps.filter(e => e.cat===cat).reduce((s,e) => s+Math.abs(e.amount), 0); }
...
function renderAuthScreen() {
  document.getElementById('app').innerHTML = `
    <div class="auth-screen">
      ...
      <div class="auth-sub">Inicia sessão com a conta que te foi criada.</div>
      <div class="auth-error" id="auth-error"></div>
      <div class="form-row"><label class="form-label">Email</label>
        <input class="form-input" id="auth-email" type="email" placeholder="email@exemplo.com" autocomplete="username"></div>
      <div class="form-row"><label class="form-label">Password</label>
        <input class="form-input" id="auth-password" type="password" placeholder="••••••••" autocomplete="current-password"></div>
...
async function handleLogin() {
  ...
  if (!email || !password) { errEl.textContent='Preenche o email e a password.'; errEl.style.display='block'; return; }
  ...
  if (error) { errEl.textContent='Email ou password incorretos.'; errEl.style.display='block'; }
}
...
function renderApp() {
  ...
  document.getElementById('app').innerHTML = `
    ...
    ${isAdmin ? `<button class="fab" onclick="openExpenseModal()">＋</button>` : ''}
    <div class="bottom-nav">
      <button class="nav-btn" id="nav-resumo"       onclick="goTo('resumo')">       📊<br>Resumo</button>
      <button class="nav-btn" id="nav-graficos"     onclick="goTo('graficos')">     📈<br>Gráficos</button>
      <button class="nav-btn" id="nav-transacoes"   onclick="goTo('transacoes')">   🧾<br>Despesas</button>
      <button class="nav-btn" id="nav-transferencias" onclick="goTo('transferencias')">⇄<br>Transf.</button>
      ${isAdmin ? `
      <button class="nav-btn" id="nav-ficheiros"    onclick="goTo('ficheiros')">    📁<br>Ficheiros</button>
      <button class="nav-btn" id="nav-importar"     onclick="goTo('importar')">     ⬆<br>Importar</button>
      <button class="nav-btn" id="nav-regras"       onclick="goTo('regras')">       ⚙︎<br>Regras</button>` : ''}
    </div>
    <div class="modal-overlay" id="modal-overlay"></div>
    <div class="toast" id="toast"></div>
  `;
  const safe = ['resumo','graficos','transacoes','transferencias','ficheiros','importar','regras'];
  goTo(safe.includes(activeScreen) && (isAdmin || !['ficheiros','importar','regras'].includes(activeScreen)) ? activeScreen : 'resumo');
}
...
function renderScreen(screen) {
  const map = { resumo: renderResumo, graficos: renderGraficos, transacoes: renderTransacoes,
                transferencias: renderTransferencias, ficheiros: renderFicheiros,
                importar: renderImportar, regras: renderRegras };
...
function renderResumo() {
  ...
  const budgetRows = getCats().map(({name: cat}) => {
    ...
    return `
      <div class="budget-row">
        <div class="budget-cat">${getCatIcon(cat)} ${cat}</div>
  ...
  document.getElementById('content').innerHTML = `
    ...
    <div class="metric-grid">
      ...
      <div class="metric"><div class="metric-label">Maior categoria</div><div class="metric-value" style="font-size:16px;">${getCatIcon(topCat)} ${topCat}</div></div>
      <div class="metric"><div class="metric-label">Poupanças</div><div class="metric-value">€${catTotal(exps,'Poupanças').toFixed(0)}</div><div class="metric-sub ok">este mês</div></div>
...
function renderGraficos() {
  ...
  const months6 = [];
  for (let i=5;i>=0;i--) { let m=currentMonth-i,y=currentYear; if(m<0){m+=12;y--;} months6.push({m,y,label:MONTHS[m].slice(0,3)}); }
  const catNames = getCatNames();
  const barData = catNames.map(cat => ({
    label: cat, data: months6.map(({m,y})=>catTotal(getMonthExpenses(m,y),cat)),
    backgroundColor: cssColor(cat), borderRadius: 4
  }));
  document.getElementById('bar-legend').innerHTML = catNames.map(c=>
    `<span><span class="legend-dot" style="background:${cssColor(c)};"></span>${getCatIcon(c)} ${c}</span>`).join('');
  if (barChartInstance) barChartInstance.destroy();
  const textColor = getComputedStyle(document.documentElement).getPropertyValue('--text-secondary').trim();
  barChartInstance = new Chart(document.getElementById('barChart'),{
    type:'bar', data:{labels:months6.map(m=>m.label),datasets:barData},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},
      scales:{x:{stacked:true,grid:{display:false},ticks:{color:textColor,font:{size:11}}},
  ...
  const pieVals = catNames.map(c=>catTotal(currExps,c));
  const pieTotal = pieVals.reduce((s,v)=>s+v,0);
  document.getElementById('pie-legend').innerHTML = catNames.map((c,i)=>
    `<span><span class="legend-dot" style="background:${cssColor(c)};"></span>${getCatIcon(c)} ${c} ${pieTotal?Math.round(pieVals[i]/pieTotal*100):0}%</span>`).join('');
  if (pieChartInstance) pieChartInstance.destroy();
  pieChartInstance = new Chart(document.getElementById('pieChart'),{
    type:'doughnut', data:{labels:catNames,datasets:[{data:pieVals,backgroundColor:catNames.map(cssColor),borderWidth:2,borderColor:'transparent'}]},
...
function renderTransacoes() {
  ...
  document.getElementById('content').innerHTML = `
    ...
    ${internalCount ? `<div class="card-flat" style="background:var(--info-bg);margin-bottom:14px;cursor:pointer;" onclick="goTo('transferencias')">
      <div style="font-size:13px;color:var(--info);font-weight:600;display:flex;align-items:center;justify-content:space-between;">
        <span>⇄ ${internalCount} transferência${internalCount>1?'s':''} entre contas excluída${internalCount>1?'s':''} dos gastos</span>
        <span>▶</span>
      </div></div>` : ''}
    ${pending.length&&isAdmin ? `<div class="section-title" style="color:var(--warning);">⚠️ Precisam de categoria (${pending.length})</div><div class="card" id="tx-pending-card"></div>` : ''}
  ...
  if (pending.length&&isAdmin) document.getElementById('tx-pending-card').innerHTML='<ul class="tx-list">'+pending.map(tx=>txHTML(tx)).join('')+'</ul>';
  document.getElementById('tx-all-card').innerHTML = sorted.length
    ? '<ul class="tx-list">'+sorted.map(tx=>txHTML(tx)).join('')+'</ul>'
    : '<div class="empty-state">Sem despesas registadas neste mês.</div>';
  if (income.length) document.getElementById('tx-income-card').innerHTML='<ul class="tx-list">'+sortedIncome.map(tx=>incomeHTML(tx)).join('')+'</ul>';
}
...
function txHTML(tx) {
  ...
  const catPicker = (!tx.cat && isAdmin) ? `
    <select class="cat-select" onchange="assignCategory('${tx.id}', this.value)">
      ...
      ${getCatNames().map(c=>`<option>${c}</option>`).join('')}
    </select>` : '';
  const editBtn = (tx.cat && isAdmin) ? `<button class="btn-edit-cat" onclick="openEditCatModal('${tx.id}','${tx.cat}')">✏️</button>` : '';
  const deleteBtn = isAdmin ? `<button class="btn-edit-cat" onclick="deleteTransaction('${tx.id}')" style="color:var(--danger);">🗑</button>` : '';
  return `
    <li class="tx-item">
      <div class="tx-icon" style="background:${col}22;">${getCatIcon(cat)}</div>
      <div class="tx-info">
...
function incomeHTML(tx) {
  ...
  return `
    <li class="tx-item">
      <div class="tx-icon" style="background:var(--success-bg);">↙</div>
      <div class="tx-info">
        <div class="tx-name">${escapeHtml(tx.desc)}</div>
...
async function assignCategory(id, cat) {
  ...
  if (!tx) return;
  const { error } = await supabaseClient.from('transactions').update({category:cat,auto_categorized:false}).eq('id',id);
  if (error) { showToast('Erro ao guardar categoria.'); return; }
  ...
  if (words.length > 0) {
    const kw = words[0];
    if (!rules[kw]) {
      const { error: re } = await supabaseClient.from('rules').insert({keyword:kw,category:cat});
      if (!re) {
        rules[kw] = cat;
        const toUpdate = transactions.filter(t => !t.cat && t.desc.toLowerCase().includes(kw));
        if (toUpdate.length) {
          await supabaseClient.from('transactions').update({category:cat,auto_categorized:true}).in('id',toUpdate.map(t=>t.id));
...
function openEditCatModal(id, currentCat) {
  document.getElementById('modal-overlay').innerHTML = `
    <div class="modal">
      <h3>Alterar categoria</h3>
      <div class="form-row"><label class="form-label">Nova categoria</label>
        <select class="form-select" id="edit-cat-sel">
          ${getCatNames().map(c=>`<option ${c===currentCat?'selected':''}>${c}</option>`).join('')}
        </select></div>
      <div class="btn-row">
        <button class="btn-cancel" onclick="closeModal()">Cancelar</button>
        <button class="btn-save" onclick="saveEditCat('${id}')">Guardar</button>
      </div>
    </div>`;
...
async function saveEditCat(id) {
  const cat = document.getElementById('edit-cat-sel').value;
  const { error } = await supabaseClient.from('transactions').update({category:cat,auto_categorized:false}).eq('id',id);
  if (error) { showToast('Erro ao guardar.'); return; }
  const tx = transactions.find(t => t.id===id);
  if (tx) { tx.cat=cat; tx.autocat=false; }
  closeModal();
  renderScreen(activeScreen);
...
async function deleteTransaction(id) {
  if (!confirm('Apagar esta transação? Esta ação não pode ser desfeita.')) return;
  const { error } = await supabaseClient.from('transactions').delete().eq('id', id);
  if (error) { showToast('Erro ao apagar transação.'); return; }
  transactions = transactions.filter(t => t.id !== id);
  renderScreen(activeScreen);
  showToast('Transação apagada.');
}
...
function renderTransferencias() {
  ...
  document.getElementById('content').innerHTML = `
    ...
    <div class="card" style="background:${balanced?'var(--success-bg)':'var(--warning-bg)'};border-color:transparent;margin-bottom:14px;">
      <div style="display:flex;align-items:center;gap:10px;">
        <span style="font-size:22px;">${balanced?'✅':'⚠️'}</span>
        <div>
          ...
          <div style="font-size:12px;color:${balanced?'var(--success)':'var(--warning)'};margin-top:2px;">
            ${balanced?'Soma: €0,00 — cada saída tem a entrada correspondente.':`Soma: €${balance.toFixed(2)} — falta a transação correspondente noutro banco.`}
          </div>
        </div>
      </div>
    </div>
  ...
  document.getElementById('transfer-list').innerHTML = allInternal.length
    ...
    : '<div class="empty-state">⇄<br>Nenhuma transferência detetada ainda.<br><small>Adiciona os teus IBANs abaixo para ativar a deteção automática.</small></div>';
  if (isAdmin) renderOwnAccountsCard();
}

function transferHTML(tx) {
  const isCredit = tx.amount > 0;
  const revertBtn = isAdmin ? `<button class="btn-outline" style="padding:5px 8px;font-size:11px;" onclick="revertInternal('${tx.id}')">Não é transferência</button>` : '';
  const deleteBtn = isAdmin ? `<button class="btn-edit-cat" onclick="deleteTransaction('${tx.id}')" style="color:var(--danger);">🗑</button>` : '';
  return `
    <li class="tx-item">
      <div class="tx-icon" style="background:var(--info-bg);">${isCredit?'↙':'↗'}</div>
      <div class="tx-info">
        <div class="tx-name">${escapeHtml(tx.desc)}</div>
        <div class="tx-meta">${tx.date} · ${tx.bank}</div>
        ${revertBtn}
    ...
    <ul class="tx-list" id="own-accounts-list">
      ${ownAccounts.map(a=>`
        <li class="tx-item">
          ...
          <button class="btn-edit-cat" onclick="deleteOwnAccount('${a.id}')" style="color:var(--danger);">🗑</button>
        </li>`).join('')}
    </ul>
    <div style="display:flex;gap:8px;margin-top:12px;">
      <input class="form-input" id="new-acc-label" placeholder="Nome (ex: Revolut)" style="flex:1;">
      <input class="form-input" id="new-acc-iban"  placeholder="Últimos 6 dígitos IBAN" style="flex:1;">
    </div>
    <button class="btn-outline" style="width:100%;margin-top:8px;" onclick="addOwnAccount()">＋ Adicionar conta</button>`;
}
...
async function addOwnAccount() {
  ...
  showToast(`Conta "${label}" adicionada.`);
}
...
function renderFicheiros() {
  ...
  transactions.forEach(tx => {
    const key = tx.bank + '|' + tx.date.slice(0,7);
    if (!grouped[key]) grouped[key] = { bank: tx.bank, month: tx.date.slice(0,7), count: 0, total: 0, ids: [] };
    grouped[key].count++;
    grouped[key].total += tx.amount;
    grouped[key].ids.push(tx.id);
  });
  const groups = Object.values(grouped).sort((a,b)=>b.month.localeCompare(a.month));

  document.getElementById('content').innerHTML = `
    <div class="section-title" style="margin-top:4px;">Transações por banco e mês</div>
    <div class="card-flat" style="margin-bottom:14px;font-size:13px;color:var(--text-secondary);">
      Aqui podes ver todas as transações agrupadas por banco e mês. Se importaste um ficheiro errado ou duplicado, apaga o grupo correspondente.
    </div>
    <div class="card" id="files-list"></div>`;

  document.getElementById('files-list').innerHTML = groups.length
    ? '<ul class="tx-list">'+groups.map(g=>`
        <li class="tx-item">
          <div class="tx-icon" style="background:var(--bg-secondary);font-size:20px;">🏦</div>
          <div class="tx-info">
            <div class="tx-name">${g.bank} — ${g.month}</div>
            <div class="tx-meta">${g.count} transações · saldo €${g.total.toFixed(2)}</div>
          </div>
          <button class="btn-outline" style="color:var(--danger);padding:6px 10px;" onclick="deleteGroup('${g.bank}','${g.month}')">🗑 Apagar</button>
        </li>`).join('')+'</ul>'
    : '<div class="empty-state">Ainda não foram importadas transações.</div>';
}

async function deleteGroup(bank, month) {
  if (!confirm(`Apagar todas as transações de "${bank}" em ${month}? Esta ação não pode ser desfeita.`)) return;
  const toDelete = transactions.filter(t => t.bank===bank && t.date.startsWith(month));
  const ids = toDelete.map(t=>t.id);
  const { error } = await supabaseClient.from('transactions').delete().in('id', ids);
  if (error) { showToast('Erro ao apagar transações.'); return; }
  transactions = transactions.filter(t => !ids.includes(t.id));
  renderFicheiros();
  showToast(`${ids.length} transações de ${bank} (${month}) apagadas.`);
}
...
function renderImportar() {
  ...
  document.getElementById('content').innerHTML = `
    ...
    <div class="drop-zone" onclick="document.getElementById('csv-input').click()">
      ...
      <div class="drop-label">Carregar ficheiro CSV ou XLS</div>
      <div class="drop-sub">${BANK_HINTS[selectedBank]}</div>
    </div>
    <div class="section-title">Ou experimenta com dados de exemplo</div>
    <button class="btn-outline" style="width:100%;" onclick="loadSample()">🗄 Carregar transações de exemplo</button>
...
function handleFile(e) {
  ...
  if (isXLS) {
    reader.onload = ev => {
      ...
      } catch(err) { console.error(err); showToast('Não foi possível ler o ficheiro XLS.'); }
    };
  ...
  el.innerHTML = `
    <div class="card" style="margin-top:16px;">
      ...
      ${internal?`<div class="card-flat" style="margin-top:10px;background:var(--info-bg);">
        <div style="font-size:13px;color:var(--info);font-weight:600;">⇄ ${internal} transferência${internal>1?'s':''} entre contas detetada${internal>1?'s':''}</div>
        <div style="font-size:12px;color:var(--info);margin-top:4px;">Não conta para os gastos. Revê em "Transf.".</div>
      </div>`:''}
    </div>
...
function renderRegras() {
  ...
  document.getElementById('content').innerHTML = `
    <div class="section-title" style="margin-top:4px;">Categorias (${getCats().length})</div>
    <div class="card" id="cats-card"></div>
    <div class="section-title">Regras aprendidas (${entries.length})</div>
...
function renderCatsCard() {
  document.getElementById('cats-card').innerHTML = `
    <ul class="tx-list">
      ${getCats().map((c,i)=>`
        <li class="tx-item">
          <div style="font-size:20px;width:34px;text-align:center;">${c.icon}</div>
          <div class="tx-info"><div class="tx-name">${escapeHtml(c.name)}</div></div>
          <div style="display:flex;gap:6px;">
            <button class="btn-edit-cat" onclick="openEditCatNameModal(${i})">✏️</button>
            ${getCats().length>1?`<button class="btn-edit-cat" onclick="deleteCat(${i})" style="color:var(--danger);">🗑</button>`:''}
          </div>
        </li>`).join('')}
    </ul>
    <div style="display:flex;gap:8px;margin-top:12px;">
      <input class="form-input" id="new-cat-icon" placeholder="Emoji (ex: 🎓)" style="width:80px;">
      <input class="form-input" id="new-cat-name" placeholder="Nome da categoria" style="flex:1;">
    </div>
    <button class="btn-outline" style="width:100%;margin-top:8px;" onclick="addCat()">＋ Adicionar categoria</button>`;
}
  ...
  document.getElementById('rules-card').innerHTML = entries.length
    ? '<ul class="tx-list">'+entries.map(([kw,cat])=>`
        <li class="tx-item">
          ...
          <button class="btn-edit-cat" onclick="deleteRule('${kw.replace(/'/g,"\\'")}')">🗑</button>
        </li>`).join('')+'</ul>'
    : '<div class="empty-state">Ainda sem regras. Categoriza uma despesa para criar a primeira.</div>';
}

function renderBudgetsCard() {
  document.getElementById('budgets-card').innerHTML = getCatNames().map(cat=>`
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
      <div style="flex:1;font-size:14px;font-weight:500;">${getCatIcon(cat)} ${cat}</div>
...
function openEditCatNameModal(idx) {
  const cat = getCats()[idx];
  document.getElementById('modal-overlay').innerHTML = `
    <div class="modal">
      <h3>Editar categoria</h3>
      <div class="form-row"><label class="form-label">Ícone (emoji)</label>
        <input class="form-input" id="edit-cat-icon" value="${cat.icon}"></div>
      <div class="form-row"><label class="form-label">Nome</label>
        <input class="form-input" id="edit-cat-name" value="${escapeHtml(cat.name)}"></div>
      <div class="btn-row">
        <button class="btn-cancel" onclick="closeModal()">Cancelar</button>
        <button class="btn-save" onclick="saveEditCatName(${idx})">Guardar</button>
      </div>
    </div>`;
  document.getElementById('modal-overlay').classList.add('open');
}

function saveEditCatName(idx) {
  ...
  customCats[idx] = { ...customCats[idx], icon: icon||'•', name };
  closeModal();
  renderRegras();
  showToast('Categoria atualizada.');
}
...
function addCat() {
  ...
  if (!name) { showToast('Escreve o nome da categoria.'); return; }
  if (getCatNames().includes(name)) { showToast('Já existe uma categoria com esse nome.'); return; }
  customCats.push({ name, icon });
  renderCatsCard();
  renderBudgetsCard();
...
function deleteCat(idx) {
  const cat = getCats()[idx];
  if (!confirm(`Apagar a categoria "${cat.name}"? As despesas com esta categoria ficam sem categoria.`)) return;
  ...
  document.getElementById('modal-overlay').innerHTML = `
    <div class="modal">
      ...
      <div class="form-row"><label class="form-label">Categoria</label>
        <select class="form-select" id="f-cat">${getCatNames().map(c=>`<option>${c}</option>`).join('')}</select></div>
...
async function saveManualExpense() {
  ...
  const tx = { date, description:desc, amount:-Math.abs(val), bank:'Manual', category:cat,
               auto_categorized:false, fingerprint:txFingerprint({date,desc,amount:-Math.abs(val),bank:'Manual'}),
